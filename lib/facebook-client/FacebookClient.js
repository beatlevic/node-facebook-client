    ...
    return function(cb) {
        ...
        var request = protocol.request(options, function(response){
            ...
            response.on("end", function () {
                ...
                cb(null, parser(body.join("")));
            });
        });

        callTimeout = setTimeout(function() {
            console.debug("[FacebookClient] TIMEOUT");
            timedOut = true;
            try {
              request.abort();
            } catch (error) {
            }
            return cb(new Error("Facebook API timeout"));
        }, 10000);
...
var FacebookClient = function(api_key, api_secret, options) {
    ...
    this.getAccessToken = function(params) {
        ...
        return function(cb) {
            doRawQueryRequest(self.options.facebook_graph_secure_server_host, self.options.facebook_graph_secure_server_port, "/oauth/access_token" + '?' +  querystring.stringify(access_params), true)(function(err, response) {
                if (!response) {
                    cb();
                } else {
